...
<!-- EAJ - 29JUL2023. Esto cambia el comportamiento de <solutions>, <hints>, <answers> en los <activity> con<statement> para que el latex tenga la las soluciones afuera del environment \begin{activity}...\end{activity}. -->
<!-- Ver las líneas con comentarios que empiezan con [EAJ -] -->
...
<xsl:template match="exercise[boolean(&INLINE-EXERCISE-FILTER;)]|&PROJECT-LIKE;|exercises//exercise|worksheet//exercise|reading-questions//exercise">
    ...
    <xsl:apply-templates select="." mode="exercise-components">
        ...
        <xsl:with-param name="b-has-statement" select="$b-has-statement" />
        <!-- EAJ - quité esto de acá y lo pasé para después del \end{} -->
        <!-- <xsl:with-param name="b-has-hint"      select="$b-has-hint" /> -->
        <!-- <xsl:with-param name="b-has-answer"    select="$b-has-answer" /> -->
        <!-- <xsl:with-param name="b-has-solution"  select="$b-has-solution" /> -->
    </xsl:apply-templates>
    ...
    <xsl:if test="$project and (statement or task)">
        <!-- EAJ - acá va lo que quité arriba -->
        <xsl:apply-templates select="." mode="exercise-components">
            <xsl:with-param name="b-original" select="true()" />
            <xsl:with-param name="b-component-heading" select="true()"/>
            <xsl:with-param name="b-has-hint"      select="$b-has-hint" />
