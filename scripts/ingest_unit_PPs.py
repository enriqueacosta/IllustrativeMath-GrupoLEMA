...
grade/unit (graX-uniY). Steps:
  ...
  4. Write one new `PP-<uuid>.ptx` file per problem into `source/content/`.
     ...
     (marking it as not yet translated). PP identifiers are generated by
     `generate_pp_code.py` — see that file for the naming convention and
     collision-probability rationale.
...
from generate_pp_code import new_pp_id
