...
<xsl:template match="xref[@ref and not(contains(normalize-space(@ref), ' ')) and  not(contains(normalize-space(@ref), ','))]">
    ...
    <xsl:choose>
        ...
        <!-- EAJ - forzar template "activity-titles" -->
        <xsl:when test="@text='activity-titles'">
            <xsl:apply-templates select="." />
        </xsl:when>
        <xsl:otherwise>
...
<!-- EAJ - NEW activity-titles -->
<xsl:template match="xref[@ref and @text='activity-titles' and not(contains(normalize-space(@ref), ' ')) and not(contains(normalize-space(@ref), ','))]">
    <!-- Resolve the target element by @ref -->
    <xsl:variable name="target" select="id(@ref)" />

    <!-- Get the shorttitle of the target element's parent -->
    <xsl:variable name="parent-title" select="$target/parent::*/shorttitle" />

    <!-- Get the shorttitle of the grandparent of the target element -->
    <xsl:variable name="grandparent-title" select="$target/parent::*/parent::*/shorttitle" />

    <!-- Debug messages -->
    <xsl:message>
        <xsl:text>Target Node: </xsl:text>
        <xsl:copy-of select="$target" />
    </xsl:message>
    <xsl:message>
        <xsl:text>Parent Title: </xsl:text>
        <xsl:value-of select="$parent-title" />
    </xsl:message>
    <xsl:message>
        <xsl:text>Grandparent Title: </xsl:text>
        <xsl:value-of select="$grandparent-title" />
    </xsl:message>

    <!-- Combine the titles -->
    <xsl:variable name="combined-title">
        <xsl:choose>
            <xsl:when test="$parent-title and $grandparent-title">
                <!-- <xsl:text>\text{</xsl:text> -->
                <xsl:value-of select="concat($grandparent-title, ' - ', $parent-title)" />
                <!-- <xsl:text>}</xsl:text> -->
            </xsl:when>
    ...
    <!-- Output the combined title as part of the xref -->
    <xsl:apply-templates select="." mode="xref-link">
        <xsl:with-param name="target" select="$target" />
        <xsl:with-param name="origin" select="'xref'" />
        <xsl:with-param name="content" select="$combined-title" />
    </xsl:apply-templates>
</xsl:template>
